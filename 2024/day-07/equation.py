        ...
        for value in self.values:
            ...
            if not parent_nodes:
                ...
                for node in parent_nodes:
                    first_node, second_node = self.perform_all_operations_on_node(current_node=node,
                                                                                  value=value)
                    temp_parent_notes.extend([first_node, second_node])
                parent_nodes = deepcopy(temp_parent_notes)
        return parent_nodes

    @staticmethod
    def perform_all_operations_on_node(current_node, value) -> tuple[int, int]:
